...
ROOT=/tmp/qckn-var-r2-eval-atlas-v1
...
arena_sha	$ARENA_SHA
purpose	post_var_active_eval_path_census
...
cd "$GITHUB_WORKSPACE"
RUSTFLAGS='-C target-cpu=native' cargo build --release --locked --features qckn-r2-eval-atlas
cp target/release/sokonanoda "$ROOT/r2-eval-atlas.bin"
sha256sum "$ROOT/r2-eval-atlas.bin" > "$ROOT/evidence/binary.sha256"

set +e
/usr/bin/time -f 'wall_s=%e\nuser_s=%U\nsys_s=%S\nmax_rss_kb=%M' -o "$ROOT/evidence/mathlib.time" \
  "$ROOT/r2-eval-atlas.bin" "$ROOT/config.json" \
...
grep '^QCKN_R2_EVAL_ATLAS' "$ROOT/evidence/mathlib.stderr" > "$ROOT/evidence/eval-atlas.tsv"
...
for line in open(sys.argv[1]):
    ...
    assert tag=="QCKN_R2_EVAL_ATLAS"
    rows[label]=int(count)

labels=[
"eval_entry","direct_var_fast","closed_cache_hit","closed_cache_miss","open_cache_hit","open_cache_miss","direct_no_cache",
"app_chain_same","app_chain_mixed","app_simple_lambda","app_simple_apply","var_remaining","sort","const","lambda","pi","let","proj","natlit","strlit"
]
...
varfast=rows["direct_var_fast"]
nocache=rows["closed_cache_miss"]+rows["open_cache_miss"]+direct
paths={k:rows[k] for k in labels[7:]}
path_total=sum(paths.values())

accounted=varfast+closed+open_+direct
print(f"eval_entry={entry}")
print(f"entry_accounted={accounted}")
assert entry == accounted, (entry,accounted)

print(f"direct_var_fast={varfast} share_of_eval={varfast/entry if entry else 0:.6f}")
...
assert rows["var_remaining"] == 0, rows["var_remaining"]
...
print("=== R2 CACHE SURFACE ===")
for label,count in [
    ("direct_var_fast",varfast),
...
ranked=[(k,v) for k,v in paths.items() if k!="var_remaining"]
ranked.sort(key=lambda kv:kv[1],reverse=True)
print(f"NEXT_PATH={ranked[0][0]}")
print(f"NEXT_PATH_COUNT={ranked[0][1]}")
print("DECISION=OBSERVATIONAL_R2_EVAL_CENSUS_ONLY__NO_INTERVENTION_PROMOTED")
PY
