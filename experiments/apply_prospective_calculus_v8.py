...
# Frozen accepted developmental prefix G1 + G2 from prior experiments.
...
# Frozen accepted G3 winner from V7: final-field Pi capability preservation.
old='''        match self.force_all(depth, cur) {\n            Value::Pi { domain, .. } => {'''
new='''        let cur_f = match cur { Value::Pi { .. } => cur, _ => self.force_all(depth, cur) };\n        match cur_f {\n            Value::Pi { domain, .. } => {'''
if old not in s: raise SystemExit('g3 final-field site missing')
s=s.replace(old,new,1)

# Prospective candidates were frozen before timing outcomes. Each is a composition
# of already verifier-safe guard-before-force operators at a distinct consumer.
def add_rigid(text):
...
def add_prior(text):
    old='''        for i in 0..idx {\n            match self.force_all(depth, cur) {'''
    new='''        for i in 0..idx {\n            let cur_f = match cur { Value::Pi { .. } => cur, _ => self.force_all(depth, cur) };\n            match cur_f {'''
    if old not in text: raise SystemExit('prior-field site missing')
    return text.replace(old,new,1)
...
if cand=='g3':
    ...
    s=add_rigid(add_param(add_prior(s)))
else:
    raise SystemExit(f'unknown candidate {cand}')

infer.write_text(s)
print(f'PROSPECTIVE_CANDIDATE={cand}')
print('FROZEN_PREFIX=G1+G2+G3_FINAL_FIELD_PI')
print('OUTCOME_UNSEEN_AT_PREDICTION_TIME=YES')
