...
raw='''        use std::hash::{Hash, Hasher};
        let mut appgen_v23_p0: u64 = 0;
        let mut h0 = std::collections::hash_map::DefaultHasher::new();
        std::mem::discriminant(struct_ty).hash(&mut h0); appgen_v23_p0 = h0.finish();
        let mut appgen_v23_q0: u64 = 0;
        let mut appgen_v23_q1: u64 = 0;
        let mut appgen_v23_empty: u8 = 0;
        if let Value::Rigid { head, spine, .. } = struct_ty {
            let mut h1 = std::collections::hash_map::DefaultHasher::new();
            std::mem::discriminant(head).hash(&mut h1); appgen_v23_q0 = h1.finish();
            let mut h2 = std::collections::hash_map::DefaultHasher::new();
            std::mem::discriminant(&**spine).hash(&mut h2); appgen_v23_q1 = h2.finish();
            if matches!(**spine, crate::value::Spine::Empty) { appgen_v23_empty = 1; }
        }
        let appgen_v23_p3: u64 = depth as u64;
        let appgen_v23_closed: u8 = if struct_ty.is_closed() {1} else {0};
        let appgen_v23_canonical: u8 = if struct_ty.is_canonical() {1} else {0};'''
def var(path):return {'root.disc':'appgen_v23_p0','root.generated0.disc':'appgen_v23_q0','root.generated1.disc':'appgen_v23_q1','context.depth':'appgen_v23_p3'}[path]
def expr_rust(e):
...
def pred_rust(p,g):
 ...
 q={'empty':'appgen_v23_empty','closed':'appgen_v23_closed','canonical':'appgen_v23_canonical','g':g};return ' && '.join(f'({q[n]} == {int(v)}u8)' for n,v in (x.split('=') for x in p.split('&')))
if mode=='probe':new=f'''        let struct_ty = self.infer_value(flag, depth, env, ctx, structure);\n{raw}\n        let struct_ty_f = self.force_all(depth, struct_ty);\n        let appgen_v23_safe = std::ptr::eq(struct_ty, struct_ty_f);\n        eprintln!("APPGEN_V23 safe={{}} p0={{}} q0={{}} q1={{}} p3={{}} spine_empty={{}} closed={{}} canonical={{}}", if appgen_v23_safe {{1}} else {{0}}, appgen_v23_p0, appgen_v23_q0, appgen_v23_q1, appgen_v23_p3, appgen_v23_empty, appgen_v23_closed, appgen_v23_canonical);'''
elif mode in ('guard','broad'):
 g=expr_rust(expr);cond=pred_rust(pred,g);new=f'''        let struct_ty = self.infer_value(flag, depth, env, ctx, structure);\n{raw}\n        let appgen_v23_generated: u8 = {g};\n        let struct_ty_f = if {cond} {{ struct_ty }} else {{ self.force_all(depth, struct_ty) }};'''
elif mode=='g3':new=site
else:raise SystemExit('unknown mode')
s=s.replace(site,new,1);infer.write_text(s)
print(f'V23_RUNTIME_MODE={mode}');print(f'V23_RUNTIME_EXPR={expr or "NONE"}');print(f'V23_RUNTIME_PREDICATE={pred or "NONE"}');print('V23_INITIAL_OPERATOR_LANGUAGE=opaque_root+generic_facts+context');print('V23_TRAVERSAL_PRIMITIVES=enter+advance+emit_disc');print('V23_COMBINED_DECOMPOSE_OPERATOR_EXPOSED=NO');print('V23_CELL_SEQUENCE_EXPOSED=NO');print('V23_SLOT_FAMILY_EXPOSED=NO');print('V23_SEMANTIC_NAMES_EXPOSED=NO')
