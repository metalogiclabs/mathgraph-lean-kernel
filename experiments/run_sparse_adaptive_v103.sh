...
ROOT=/tmp/sparse-adaptive-v103
rm -rf "$ROOT"
mkdir -p "$ROOT/out"

echo "V103_HEAD=$(git rev-parse HEAD)"
echo "V103_BASE=$BASE"
echo "V103_ARENA=$ARENA"
grep -E "MemTotal|SwapTotal" /proc/meminfo | sed 's/^/V103_HOST_/'

# Candidate semantic gate before spending benchmark time.
cargo test --release --locked -q
echo "V103_RUST_TESTS=PASS"
...
build_pgo () {
  ...
  echo "V103_PGO_$tag=PASS"
}

build_pgo "$ROOT/base" V102
build_pgo "$GITHUB_WORKSPACE" V103

# GitHub-hosted runners usually lack a hardware PMU. Detect support honestly.
PERF_OK=0
...
run_one () {
  ...
  echo "V103_BEGIN tag=$tag test=$test"
  set +e
  ...
  echo "V103_RESULT tag=$tag test=$test rc=$rc wall=$wall rss_kb=$rss instructions=$instr"
...
for t in perf/app-lam perf/beta-ladder perf/let-ladder perf/shift-cascade perf/magma-list-deep-n36; do
  ...
  run_one V103 "$GITHUB_WORKSPACE/target/release/sokonanoda" "$t" 600
done
run_one V102 "$ROOT/base/target/release/sokonanoda" con-leche 1800
run_one V103 "$GITHUB_WORKSPACE/target/release/sokonanoda" con-leche 1800
run_one V102 "$ROOT/base/target/release/sokonanoda" mathlib 3600
run_one V103 "$GITHUB_WORKSPACE/target/release/sokonanoda" mathlib 3600
...
print("V103_SUMMARY_BEGIN")
for t,p in by.items():
    a,b=p["V102"],p["V103"]
    rss_delta=(b["rss_kb"]-a["rss_kb"])/a["rss_kb"]*100 if a["rss_kb"] else 0
    print(f"{t}: v102_wall={a['wall']} v103_wall={b['wall']} v102_rss={a['rss_kb']} v103_rss={b['rss_kb']} rss_delta_pct={rss_delta:.2f} v102_instr={a['instructions']} v103_instr={b['instructions']}")
print("V103_SUMMARY_END")
PY

echo "V103_COMPLETE=PASS"
