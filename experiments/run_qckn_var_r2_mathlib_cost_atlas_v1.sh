...
ROOT=/tmp/qckn-var-r2-mathlib-cost-atlas-v1
...
arena_sha	$ARENA_SHA
purpose	full_mathlib_qckn_active_var_cost_atlas
authority	callgrind_Ir_development_proxy
promotion_authority	Arena_perf_retired_instructions
EOF
...
cp target/release/sokonanoda "$ROOT/active-profile.bin"
sha256sum "$ROOT/active-profile.bin" > "$ROOT/evidence/binary.sha256"
...
/usr/bin/time -f 'wall_s=%e\nuser_s=%U\nsys_s=%S\nmax_rss_kb=%M' -o "$ROOT/evidence/mathlib.time" \
  timeout 7200s "$VALGRIND" --tool=callgrind --quiet --collect-jumps=no \
    ...
    "$ROOT/active-profile.bin" "$ROOT/config.json" \
...
"$ANNOTATE" --inclusive=no --threshold=0.05 "$ROOT/evidence/mathlib.callgrind" \
  > "$ROOT/evidence/mathlib.self.txt"
"$ANNOTATE" --inclusive=yes --threshold=0.05 "$ROOT/evidence/mathlib.callgrind" \
  > "$ROOT/evidence/mathlib.inclusive.txt"
...
echo 'DECISION=PROFILE_CURRENT_ACTIVE_LEADER__DERIVE_ONE_COST_RESIDUAL_ONLY' | tee -a "$ROOT/evidence/summary.txt"
