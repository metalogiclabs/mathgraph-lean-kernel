...
# v66 candidate.
# v65 found one-dependency Apps are 21.19% of Mathlib open-eval misses.
# Current Soko already carries open_eval_seen. On the first sight of an expression,
# there cannot yet be an open_eval_cache entry for that expression under any env.
# For dep1 Apps only, prove that impossible-hit case with the existing seen set and
# bypass key_env + open-cache lookup + insertion entirely. All later sightings use
# the unchanged Soko path. No new cache or representation is introduced.
...
new='''        if matches!(
            ...
            if dep1_app && self.tc_cache.open_eval_seen.insert(e) {
...
for t in std cedar mathlib; do
  ...
  echo "V66_${t^^}_SEMANTIC_REPLAY=EXACT"
done
...
rows=list(csv.DictReader(open('/tmp/v66/timings.csv')))
d=defaultdict(list)
...
for t in ('cedar','mathlib'):
    ...
    delta=(mb/ma-1)*100
    deltas.append(delta)
    print(f'V66_{t.upper()}_INCUMBENT_RUNS={a}')
    print(f'V66_{t.upper()}_CANDIDATE_RUNS={b}')
    print(f'V66_{t.upper()}_INCUMBENT_MEDIAN={ma:.3f}')
    print(f'V66_{t.upper()}_CANDIDATE_MEDIAN={mb:.3f}')
    print(f'V66_{t.upper()}_DELTA_PCT={delta:.4f}')
geo=(math.prod([1+x/100 for x in deltas])**0.5-1)*100
print(f'V66_PAIR_GEOMEAN_DELTA_PCT={geo:.4f}')
if geo <= -1.0 and max(deltas) <= 0.5:
    print('DECISION=V66_FIRST_SIGHT_BYPASS_MATERIAL__COMPOUND_ON_PROMOTED_INCUMBENT')
elif geo <= -0.5 and max(deltas) <= 0.5:
    print('DECISION=V66_FIRST_SIGHT_BYPASS_WEAK_POSITIVE__REPEAT_BEFORE_COMPOUND')
else:
    print('DECISION=V66_FIRST_SIGHT_BYPASS_NOT_MATERIAL__KILL_KEEP_PROMOTED_INCUMBENT')
print('RULE=PROVE_CACHE_HIT_IMPOSSIBLE_BEFORE_ENTERING_KEY_AND_CACHE_MACHINERY')
PY
