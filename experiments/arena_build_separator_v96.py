...
PATCH_SCRIPT_BLOB = 'b28fcc231d3dd2237de37a2ee824a6a41f5dd969'
...
def apply_exact_v93_patch(source):
    # Reuse the exact v94 reproduction of v93, not a newly invented fast path.
    script = Path(__file__).with_name('run_direct_framed_retain_and_profile_v94.sh')
    raw = script.read_bytes()
    assert git_blob(raw) == PATCH_SCRIPT_BLOB
    block = raw.decode().split('python3 - "$ROOT/candidate" <<\'PY\'\n', 1)[1].split('\nPY\n', 1)[0]
    values = {}
    for node in ast.parse(block).body:
        if isinstance(node, ast.Assign) and len(node.targets) == 1 and isinstance(node.targets[0], ast.Name):
            if node.targets[0].id in ('needle', 'fast'):
                values[node.targets[0].id] = ast.literal_eval(node.value)
    needle, fast = values['needle'], values['fast']
    text = source.read_text()
    assert text.count(needle) == 1
    source.write_text(text.replace(needle, fast, 1))


def build(arm, mode, arena):
    src = ROOT / arm
    q = shlex.quote
    if mode == 'native':
        shell('cd ' + q(str(src)) + " && RUSTFLAGS='-C target-cpu=native' cargo build --release --locked -q", arena)
    else:
        # Exact Arena PGO flags and training input; --locked prevents dependency drift.
        (src / 'pgo').mkdir(exist_ok=True)
...
def measure(mode, binaries):
    ...
    for corpus in CORPORA:
        ...
        print('V96_' + mode.upper().replace('-', '_') + '_' + corpus.upper() + '_EXACT_REPLAY=PASS', flush=True)
        ...
        print('V96_' + mode.upper().replace('-', '_') + '_' + corpus.upper() + '_WALL_DELTA_PERCENT=' + f'{delta:.6f}', flush=True)
...
def official_metrics(arena, binaries):
    ...
    for corpus in CORPORA:
        ...
        for arm in ('control', 'candidate'):
            ...
            try:
                pair[arm] = lka.run_checker_on_test(checker, tests[corpus], arena / '_build/checkers', arena / '_build/tests', ROOT / 'arena-results')
            except Exception as exc:
                pair[arm] = {'error': repr(exc), 'instructions': 0}
        ci = pair['control'].get('instructions', 0)
        xi = pair['candidate'].get('instructions', 0)
        if ci > 0 and xi > 0 and pair['control'].get('correctness') == 'correct' and pair['candidate'].get('correctness') == 'correct':
            ...
            print('V96_' + corpus.upper() + '_INSTRUCTIONS=UNAVAILABLE_OR_INCORRECT', flush=True)
        out[corpus] = pair
        STATE['official_metrics'] = out
        save()
    return out
...
def main():
    ...
    save()
    native = {arm: build(arm, 'native', arena) for arm in ('control', 'candidate')}
    measure('native', native)
    pgo = {arm: build(arm, 'arena-pgo', arena) for arm in ('control', 'candidate')}
