...
for t in perf/app-lam perf/beta-ladder perf/let-ladder con-leche; do
  ...
  while IFS= read -r line; do
    printf 'V109_RESULT test=%s %s\n' "$t" "$line"
  done < <(grep -E 'V107_CENSUS|V108_CELL|V109_CELL' "$ROOT/out/$safe.stderr")
done

echo "V109_COMPLETE=PASS"
