...
Rows are verifier-labelled anonymous binary feature vectors.  Semantic RigidHead names are
never exposed to this learner.  Conjunctions are enumerated by description length; selection
is entirely by verifier outcome and coverage.
"""
...
def candidates(nfeat: int, max_lits: int):
    ...
    for k in range(1,max_lits+1):
        for combo in itertools.combinations(atoms,k):
            if len({x.idx for x in combo}) != len(combo): continue
...
def main():
    ...
    if safe==0 or unsafe==0:
        print('GENERATIVE_APPLICABILITY_V13=UNDERIDENTIFIED'); return 2

    licensed=[]
    for p in candidates(nfeat,max_lits):
        cs=sum(1 for _,_,y,x in rows if y and p.holds(x))
        cu=sum(1 for _,_,y,x in rows if not y and p.holds(x))
        if cs>0 and cu==0: licensed.append((p.cost,-cs,p,cs))
    if not licensed:
        print('V13_NO_LICENSED_PREDICATE=1')
        print('GENERATIVE_APPLICABILITY_V13=UNDERIDENTIFIED'); return 2
    licensed.sort(key=lambda z:(z[0],z[1],z[2].encode()))
    best_cost=licensed[0][0]; best_cov=-licensed[0][1]
    minima=[z for z in licensed if z[0]==best_cost and -z[1]==best_cov]
    behavioural=[]
    for z in minima:
        sig=tuple(z[2].holds(x) for *_,x in rows)
        if not any(sig==s for s,_ in behavioural): behavioural.append((sig,z))
    ...
    print('V13_BROAD_PREDICATE='+q.encode())
    print('V13_UNIQUE_MINIMUM_GENERATED_PREDICATE=PASS')
    open('/tmp/v13_predicate.txt','w').write(p.encode())
    open('/tmp/v13_broad_predicate.txt','w').write(q.encode())
    with open('/tmp/v13_predicate.json','w') as f:
        json.dump({'predicate':[[l.idx,l.val] for l in p.lits], 'broad':[[l.idx,l.val] for l in q.lits]},f)
