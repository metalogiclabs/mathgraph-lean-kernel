...
ROOT=/tmp/bounded-sparse-v105
rm -rf "$ROOT"
mkdir -p "$ROOT/out"

echo "V105_HEAD=$(git rev-parse HEAD)"
echo "V105_BASE=$BASE"
echo "V105_ARENA=$ARENA"
grep -E "MemTotal|SwapTotal" /proc/meminfo | sed 's/^/V105_HOST_/'

python3 scripts/apply_bounded_sparse_v105.py
cargo test --release --locked -q
echo "V105_RUST_TESTS=PASS"
...
build_pgo () {
  ...
  echo "V105_PGO_$tag=PASS"
}

build_pgo "$ROOT/base" V102
build_pgo "$GITHUB_WORKSPACE" V105
...
run_one () {
  ...
  echo "V105_BEGIN tag=$tag test=$test"
  set +e
  ...
  echo "V105_RESULT tag=$tag test=$test rc=$rc wall=$wall rss_kb=$rss instructions=$instr"
...
for t in perf/app-lam perf/beta-ladder perf/let-ladder perf/shift-cascade perf/magma-list-deep-n36; do
  ...
  run_one V105 "$GITHUB_WORKSPACE/target/release/sokonanoda" "$t" 600
done
run_one V102 "$ROOT/base/target/release/sokonanoda" con-leche 1800
run_one V105 "$GITHUB_WORKSPACE/target/release/sokonanoda" con-leche 1800
run_one V102 "$ROOT/base/target/release/sokonanoda" mathlib 3600
run_one V105 "$GITHUB_WORKSPACE/target/release/sokonanoda" mathlib 3600
...
print("V105_SUMMARY_BEGIN")
for t,p in by.items():
    a,b=p["V102"],p["V105"]
    rss_delta=(b["rss_kb"]-a["rss_kb"])/a["rss_kb"]*100 if a["rss_kb"] else 0
    print(f"{t}: v102_wall={a['wall']} v105_wall={b['wall']} v102_rss={a['rss_kb']} v105_rss={b['rss_kb']} rss_delta_pct={rss_delta:.2f} v102_instr={a['instructions']} v105_instr={b['instructions']}")
print("V105_SUMMARY_END")
PY

echo "V105_COMPLETE=PASS"
