...
# Freeze the Arena version used by v80, so the cost sample is comparable to its counts.
...
PY
# First validate each frozen prefix with the original executable. No semantic edits are made.
...
# Callgrind is a deterministic instruction-count fallback when hosted perf is denied.
# Use the same optimized checker with debug information; do not add counters or noinline guards.
# Each process is bounded. A timeout is an incomplete profile, never a successful measurement.
for corpus in std cedar mathlib; do
  set +e
...
print('DECISION=UNINSTRUMENTED_COST_PROFILE__NO_REPAIR_PROMOTED')
