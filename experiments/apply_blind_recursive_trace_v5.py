...
// Frozen anonymous producer-coordinate grammar.  The offline developmental
// mechanism receives only a0..a3 and q0..q2; semantic names are not emitted.
#[inline]
...
new='''        let struct_ty = self.infer_value(flag, depth, env, ctx, structure);\n        crate::msi_obs::emit(struct_ty, 2, u8::from(matches!(struct_ty, Value::Rigid { head: RigidHead::Inductive(..), .. })));\n        let struct_ty_f = self.force_all(depth, struct_ty);'''
assert old in s
s=s.replace(old,new,1)
infer.write_text(s)
