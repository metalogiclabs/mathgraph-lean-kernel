...
FREEZE=1fc7f1aea30a8f5de7541bfa4d718d9002bed4d4
ARENA=ac1c13762de41b594fa24b90ede8cfd97ac6a765
ROOT=/tmp/mda-auto-train-v1
...
git diff --exit-code "$FREEZE" -- mda/AUTONOMOUS_CHECKER_GENESIS_V1.md
git diff --quiet "$BASE" -- src/eval.rs src/main.rs
echo "MDA_AUTO_TRAIN_FREEZE=PASS"
...
cd "$ROOT/arena"
for t in "${TRAIN[@]}"; do
  echo "MDA_AUTO_BUILD_TRAIN=$t"
  nix develop -c ./lka.py build-test "$t" >/dev/null
done
cd "$GITHUB_WORKSPACE"

# Observe real cache behavior on training only.
python3 scripts/mda_autonomous_cache_genesis_v1.py instrument src/eval.rs src/main.rs
...
for t in "${TRAIN[@]}"; do
  ...
  MDA_CACHE_CENSUS=1 "$BIN/instrumented" "$ROOT/config.json"     < "$ROOT/arena/_build/tests/$t.ndjson"     > "$ROOT/census/$safe.stdout" 2> "$ROOT/census/$safe.stderr"
  echo "MDA_AUTO_CENSUS_PASS=$t"
done

python3 scripts/mda_autonomous_cache_genesis_v1.py parse-census   "$EVIDENCE/training-census.json" "$ROOT"/census/*.stderr
python3 scripts/mda_autonomous_cache_genesis_v1.py make-policies   "$EVIDENCE/training-census.json" "$POLICY_DIR"
...
while IFS= read -r label; do
  [ -n "$label" ] || continue
  echo "MDA_AUTO_POLICY_BEGIN=$label"
  cp "$ROOT/clean-main.rs" src/main.rs
  python3 scripts/mda_autonomous_cache_genesis_v1.py apply-policy     "$ROOT/clean-eval.rs" "$POLICY_DIR/$label.json" src/eval.rs
  set +e
  build_current
  brc=$?
  set -e
  ...
  trc=$?
  set -e
  echo "MDA_AUTO_POLICY_END=$label rc=$trc"
done < "$POLICY_DIR/labels.txt"

set +e
python3 scripts/mda_autonomous_cache_genesis_v1.py select   "$EVIDENCE/training-timings.tsv" "$POLICY_DIR" "$EVIDENCE/selection.json"
rc=$?
set -e

if [ "$rc" -ne 0 ]; then
  echo "MDA_AUTO_TRAIN_VERDICT=UNKNOWN_NO_TRAINING_WIN"
  exit 0
fi
...
echo "MDA_AUTO_TRAIN_SELECTED=$SELECTED"
python3 - "$EVIDENCE/selected-policy.json" <<'PY'
import json,sys
d=json.load(open(sys.argv[1]))
print("MDA_AUTO_SELECTED_POLICY_JSON="+json.dumps(d,separators=(",",":"),sort_keys=True))
PY
python3 - "$EVIDENCE/selection.json" <<'PY'
import json,sys
d=json.load(open(sys.argv[1]))
s=d["selected"]
print("MDA_AUTO_SELECTED_METRICS="+json.dumps(s,separators=(",",":"),sort_keys=True))
PY
echo "MDA_AUTO_TRAIN_VERDICT=SELECTED_FOR_SEMANTIC_GATE"
