...
ROOT=${V86_ROOT:-/tmp/v86}
mkdir -p "$ROOT"
python3 -m unittest discover -s "$SCRIPT_DIR/../msi_v85" -p 'test_*.py'
python3 "$SCRIPT_DIR/runner.py" --selftest
...
cd "$ROOT/arena"
exec nix develop -c python3 "$SCRIPT_DIR/runner.py" --root "$ROOT/results" --source "$ROOT/base" --arena "$ROOT/arena"
