...
ROOT=/tmp/qckn-mathlib-leader-cost-atlas-v1
...
arena_sha	$ARENA_SHA
purpose	full_mathlib_current_leader_cost_atlas
...
cp target/release/sokonanoda "$ROOT/leader-profile.bin"
sha256sum "$ROOT/leader-profile.bin" > "$ROOT/evidence/binary.sha256"
...
/usr/bin/time -f 'wall_s=%e\nuser_s=%U\nsys_s=%S\nmax_rss_kb=%M' -o "$ROOT/evidence/mathlib.time" \
  timeout 7200s "$VALGRIND" --tool=callgrind --quiet --collect-jumps=no \
    ...
    "$ROOT/leader-profile.bin" "$ROOT/config.json" \
