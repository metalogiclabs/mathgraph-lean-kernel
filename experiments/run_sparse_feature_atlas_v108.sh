...
ROOT=/tmp/sparse-feature-atlas-v108
rm -rf "$ROOT"
mkdir -p "$ROOT/out"
...
for t in perf/app-lam perf/beta-ladder perf/let-ladder con-leche; do
  ...
  while IFS= read -r line; do
    printf 'V108_RESULT test=%s %s\n' "$t" "$line"
  done < <(grep -E 'V107_CENSUS|V108_CELL' "$ROOT/out/$safe.stderr")
done

echo "V108_COMPLETE=PASS"
