...
ROOT=/tmp/mda-auto-heldout-v2
HELD=(mathlib perf/grind-ring-5 perf/shift-cascade)
rm -rf "$ROOT"
mkdir -p "$ROOT/evidence" "$ROOT/bin"

CANDIDATE="$(git rev-parse HEAD)"
echo "MDA_V2_HELDOUT_CANDIDATE=$CANDIDATE"
test -f mda/generated/autonomous_cache_admission_v2/HELDOUT_AUTHORIZED
...
out={"schema":"mda-autonomous-checker-heldout-v2","base":base,"candidate":cand,
...
print("MDA_V2_HELDOUT_RESULT="+json.dumps(out,sort_keys=True))
if not passed: raise SystemExit(1)
PY

echo "MDA_V2_HELDOUT_VERDICT=PROMOTION_PERFORMANCE_PASS"
echo "MDA_V2_ABLATION_VERDICT=RESTORING_BASE_REMOVES_GENERATED_MECHANISM"
