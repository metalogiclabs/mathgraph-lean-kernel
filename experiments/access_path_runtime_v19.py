...
raw='''        use std::hash::{Hash, Hasher};
        let mut appgen_v19_p0: u64 = 0;
        let mut h0 = std::collections::hash_map::DefaultHasher::new();
        std::mem::discriminant(struct_ty).hash(&mut h0); appgen_v19_p0 = h0.finish();
        let mut appgen_v19_p1: u64 = 0;
        let mut appgen_v19_p2: u64 = 0;
        let mut appgen_v19_empty: u8 = 0;
        if let Value::Rigid { head, spine, .. } = struct_ty {
            let mut h1 = std::collections::hash_map::DefaultHasher::new();
            std::mem::discriminant(head).hash(&mut h1); appgen_v19_p1 = h1.finish();
            let mut h2 = std::collections::hash_map::DefaultHasher::new();
            std::mem::discriminant(&**spine).hash(&mut h2); appgen_v19_p2 = h2.finish();
            if matches!(**spine, crate::value::Spine::Empty) { appgen_v19_empty = 1; }
        }
        let appgen_v19_p3: u64 = depth as u64;
        let appgen_v19_closed: u8 = if struct_ty.is_closed() {1} else {0};
        let appgen_v19_canonical: u8 = if struct_ty.is_canonical() {1} else {0};'''

def path_var(path):
    return {'root.disc':'appgen_v19_p0','root.rigid.head.disc':'appgen_v19_p1','root.rigid.spine.disc':'appgen_v19_p2','context.depth':'appgen_v19_p3'}[path]
...
def pred_rust(p,ge):
    ...
    for lit in p.split('&'):
        ...
        q={'empty':'appgen_v19_empty','closed':'appgen_v19_closed','canonical':'appgen_v19_canonical','g':ge}[n]
        terms.append(f'({q} == {v}u8)')
    return ' && '.join(terms)
if mode=='probe':
    new=f'''        let struct_ty = self.infer_value(flag, depth, env, ctx, structure);\n{raw}\n        let struct_ty_f = self.force_all(depth, struct_ty);\n        let appgen_v19_safe = std::ptr::eq(struct_ty, struct_ty_f);\n        eprintln!("APPGEN_V19 safe={{}} p0={{}} p1={{}} p2={{}} p3={{}} spine_empty={{}} closed={{}} canonical={{}}", if appgen_v19_safe {{1}} else {{0}}, appgen_v19_p0, appgen_v19_p1, appgen_v19_p2, appgen_v19_p3, appgen_v19_empty, appgen_v19_closed, appgen_v19_canonical);'''
elif mode in ('guard','broad'):
    ge=expr_rust(expr); cond=pred_rust(predicate,ge)
    new=f'''        let struct_ty = self.infer_value(flag, depth, env, ctx, structure);\n{raw}\n        let appgen_v19_generated: u8 = {ge};\n        let struct_ty_f = if {cond} {{ struct_ty }} else {{ self.force_all(depth, struct_ty) }};'''
elif mode=='g3': new=site
else: raise SystemExit('unknown mode')
s=s.replace(site,new,1); infer.write_text(s)
print(f'V19_RUNTIME_MODE={mode}')
print(f'V19_RUNTIME_EXPR={expr or "NONE"}')
print(f'V19_RUNTIME_PREDICATE={predicate or "NONE"}')
print('V19_ACCESS_PATH_GRAMMAR=root,rigid.head,rigid.spine,context.depth')
print('V19_SEMANTIC_VARIANT_NAMES_EXPOSED_TO_LEARNER=NO')
