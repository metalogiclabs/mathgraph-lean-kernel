...
ROOT="/tmp/mda-warm-cache-v4-${MODE}"
rm -rf "$ROOT"
...
cd "$ROOT/arena"
for t in init-prelude extra-rec rec-missing-ih proj-of-stuck-prop proj-of-subst-prop mathlib con-leche perf/app-lam perf/beta-ladder perf/let-ladder; do
...
"""
elif mode=="warm-with-proj-cache":
    new="""        if k > 64 {
            let ck = (env as *const value::Env<'t> as usize, e);
            if let Some(r) = self.tc_cache.wide_prune_cache.get(&ck) {
                return *r;
...
echo "MDA_WARM_V4_TESTS mode=$MODE PASS"
...
echo "MDA_WARM_V4_PGO mode=$MODE PASS"
...
for t in perf/app-lam perf/beta-ladder perf/let-ladder; do run_sem "$t" 0; done
echo "MDA_WARM_V4_PROTECTED mode=$MODE PASS"
...
print("MDA_WARM_V4_CONLECHE="+json.dumps(out,sort_keys=True))
PY
...
for line in (root/"evidence/mathlib.tsv").read_text().splitlines():
    ...
    rows.append(row)
summary={"schema":"mda-warm-cache-v4","mode":mode,"mathlib":rows,
...
echo "MDA_WARM_V4_COMPLETE mode=$MODE PASS"
