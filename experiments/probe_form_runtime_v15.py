...
# Frozen accepted G1+G2+G3 prefix.
repls=[
...
raw='''        let mut appgen_v15_hash: u64 = 0;\n        let mut appgen_v15_empty: u8 = 0;\n        if let Value::Rigid { head, spine, .. } = struct_ty {\n            use std::hash::{Hash, Hasher};\n            let mut h = std::collections::hash_map::DefaultHasher::new();\n            std::mem::discriminant(head).hash(&mut h);\n            appgen_v15_hash = h.finish();\n            if matches!(**spine, crate::value::Spine::Empty) { appgen_v15_empty = 1; }\n        }\n        let appgen_v15_closed: u8 = if struct_ty.is_closed() {1} else {0};\n        let appgen_v15_canonical: u8 = if struct_ty.is_canonical() {1} else {0};'''

def expr_rust(e):
    if not e: return '0u8'
    m=re.fullmatch(r'bit\(disc_hash,(\d+)\)',e)
    if m: return f'(((appgen_v15_hash >> {int(m.group(1))}) & 1) as u8)'
    m=re.fullmatch(r'eq\(disc_hash,(\d+)\)',e)
    if m: return f'(if appgen_v15_hash == {int(m.group(1))}u64 {{1u8}} else {{0u8}})'
    m=re.fullmatch(r'neq\(disc_hash,(\d+)\)',e)
    if m: return f'(if appgen_v15_hash != {int(m.group(1))}u64 {{1u8}} else {{0u8}})'
...
def pred_rust(p,ge):
    ...
    for lit in p.split('&'):
        ...
        if n=='empty': q='appgen_v15_empty'
        elif n=='closed': q='appgen_v15_closed'
        elif n=='canonical': q='appgen_v15_canonical'
        elif n=='g': q=ge
...
if mode=='probe':
    new=f'''        let struct_ty = self.infer_value(flag, depth, env, ctx, structure);\n{raw}\n        let struct_ty_f = self.force_all(depth, struct_ty);\n        let appgen_v15_safe = std::ptr::eq(struct_ty, struct_ty_f);\n        eprintln!("APPGEN_V15 safe={{}} disc_hash={{}} spine_empty={{}} closed={{}} canonical={{}}", if appgen_v15_safe {{1}} else {{0}}, appgen_v15_hash, appgen_v15_empty, appgen_v15_closed, appgen_v15_canonical);'''
elif mode in ('guard','broad'):
    ge=expr_rust(expr)
    cond=pred_rust(predicate,ge)
    new=f'''        let struct_ty = self.infer_value(flag, depth, env, ctx, structure);\n{raw}\n        let appgen_v15_generated: u8 = {ge};\n        let struct_ty_f = if {cond} {{ struct_ty }} else {{ self.force_all(depth, struct_ty) }};'''
elif mode=='g3':
    new=site
else: raise SystemExit('unknown mode')

s=s.replace(site,new,1); infer.write_text(s)
print(f'V15_RUNTIME_MODE={mode}')
print(f'V15_RUNTIME_EXPR={expr or "NONE"}')
print(f'V15_RUNTIME_PREDICATE={predicate or "NONE"}')
print('V15_SEMANTIC_VARIANT_NAMES_EXPOSED_TO_LEARNER=NO')
