...
#[derive(Default)]
struct BqClass {
    keys: HashSet<(usize, u64)>,
    vals: HashSet<usize>,
    pairs: HashSet<(usize, u64, usize)>,
}
...
fn bq_record<'t>(env: E<'t>, e: ExprPtr<'t>, v: V<'t>) {
    // Deterministic 1/1024 key sample: every occurrence of a sampled key is retained,
    // avoiding event-frequency bias in the distinct-state census.
    let ep = e.get_hash();
    ...
    st.all.keys.insert((envp, ep));
    st.all.vals.insert(vp);
    st.all.pairs.insert((envp, ep, vp));
    st.classes[c].keys.insert((envp, ep));
    st.classes[c].vals.insert(vp);
    st.classes[c].pairs.insert((envp, ep, vp));
}
...
pub fn print_behavioural_quotient_census() {
    ...
    eprintln!("MSI_BQ_CENSUS {{\\\"sample_rate\\\":1024,\\\"all\\\":{},\\\"app\\\":{},\\\"proj\\\":{},\\\"let\\\":{},\\\"pi\\\":{},\\\"lambda\\\":{}}}",
...
print('applied MSI behavioural quotient census v9')
