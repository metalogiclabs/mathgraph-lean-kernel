...
echo "V100_FINAL_HEAD=$(git rev-parse HEAD)"
grep -E "MemTotal|SwapTotal" /proc/meminfo | sed "s/^/V100_FINAL_HOST_/"

# Full source test suite first.
cargo test --release --locked -q

# Freeze the live small Arena corpus and metadata.
...
print("V100_FINAL_LIVE_ARENA_REV="+str(m.get("git_revision")))
print("V100_FINAL_LIVE_ARENA_RUN="+str(m.get("github_run_id")))
PY
tar -xzf "$ROOT/tests.tar.gz" -C "$ROOT/tests"

# Exact Arena checker config: four threads and PGO trained on init-prelude.
...
echo "V100_FINAL_PGO_BUILD=PASS"

# Run every downloadable good/bad test under the exact four-thread candidate.
...
for expected,dirname,want in [("accept","good",0),("reject","bad",1)]:
  for p in sorted((root/"tests"/dirname).rglob("*.ndjson")):
    ...
    if cp.returncode != want:
      failures.append(row); print("V100_FINAL_SMALL_FAIL="+json.dumps(row),flush=True)
(root/"small-gate.json").write_text(json.dumps({"count":len(rows),"failures":failures,"rows":rows},indent=2))
print(f"V100_FINAL_SMALL_COUNT={len(rows)}")
print(f"V100_FINAL_SMALL_FAILURES={len(failures)}")
if failures: raise SystemExit(1)
print("V100_FINAL_SMALL_GATE=PASS")
PY

# Explicitly cover the six post-snapshot equational-theories tests.
cd "$ROOT/arena"
...
for t in perf/magma-list-pair-n7 perf/magma-list-pair-n21 perf/magma-list-deep-n21 perf/magma-list-deep-n36 perf/magma-string-n4 perf/magma-string-pair-n9; do
  ...
  echo "V100_FINAL_SIX_BEGIN=$t"
  set +e; target/release/sokonanoda "$ROOT/config.json" < "$f" >/dev/null 2>"$ROOT/out/$(basename "$t").err"; rc=$?; set -e
  echo "V100_FINAL_SIX_EXIT_$t=$rc"
  [ "$rc" -eq 0 ] || fails=$((fails+1))
done
echo "V100_FINAL_SIX_FAILURES=$fails"
[ "$fails" -eq 0 ] || exit 1
echo "V100_FINAL_SIX_GATE=PASS"

# Decisive giant workload, under the same PGO build and 4-thread config Arena uses.
cd "$ROOT/arena"
nix develop -c ./lka.py build-test con-leche >/dev/null
cd "$GITHUB_WORKSPACE"
echo "V100_FINAL_CONLECHE_BEGIN"
...
grep "Maximum resident set size" "$ROOT/out/conleche.err" | sed "s/^/V100_FINAL_CONLECHE_/"
grep "Elapsed (wall clock) time" "$ROOT/out/conleche.err" | sed "s/^/V100_FINAL_CONLECHE_/"
echo "V100_FINAL_CONLECHE=PASS"
echo "V100_FINAL_COMPLETE=PASS"
