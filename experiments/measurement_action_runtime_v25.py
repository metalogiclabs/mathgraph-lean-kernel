...
raw='''        use std::hash::{Hash, Hasher};
        let mut appgen_v25_p0: u64 = 0;
        let mut h0 = std::collections::hash_map::DefaultHasher::new();
        std::mem::discriminant(struct_ty).hash(&mut h0); appgen_v25_p0 = h0.finish();
        let mut appgen_v25_q0: u64 = 0;
        let mut appgen_v25_q1: u64 = 0;
        let mut appgen_v25_empty: u8 = 0;
        if let Value::Rigid { head, spine, .. } = struct_ty {
            let mut h1 = std::collections::hash_map::DefaultHasher::new();
            std::mem::discriminant(head).hash(&mut h1); appgen_v25_q0 = h1.finish();
            let mut h2 = std::collections::hash_map::DefaultHasher::new();
            std::mem::discriminant(&**spine).hash(&mut h2); appgen_v25_q1 = h2.finish();
            if matches!(**spine, crate::value::Spine::Empty) { appgen_v25_empty = 1; }
        }
        let appgen_v25_p3: u64 = depth as u64;
        let appgen_v25_closed: u8 = if struct_ty.is_closed() {1} else {0};
        let appgen_v25_canonical: u8 = if struct_ty.is_canonical() {1} else {0};'''
def var(path):return {'root.disc':'appgen_v25_p0','root.generated0.disc':'appgen_v25_q0','root.generated1.disc':'appgen_v25_q1','context.depth':'appgen_v25_p3'}[path]
def expr_rust(e):
...
def pred_rust(p,g):
 ...
 q={'empty':'appgen_v25_empty','closed':'appgen_v25_closed','canonical':'appgen_v25_canonical','g':g};return ' && '.join(f'({q[n]} == {int(v)}u8)' for n,v in (x.split('=') for x in p.split('&')))
if mode=='probe':new=f'''        let struct_ty = self.infer_value(flag, depth, env, ctx, structure);\n{raw}\n        let struct_ty_f = self.force_all(depth, struct_ty);\n        let appgen_v25_safe = std::ptr::eq(struct_ty, struct_ty_f);\n        eprintln!("APPGEN_V25 safe={{}} p0={{}} q0={{}} q1={{}} p3={{}} spine_empty={{}} closed={{}} canonical={{}}", if appgen_v25_safe {{1}} else {{0}}, appgen_v25_p0, appgen_v25_q0, appgen_v25_q1, appgen_v25_p3, appgen_v25_empty, appgen_v25_closed, appgen_v25_canonical);'''
elif mode in ('guard','broad'):
 g=expr_rust(expr);cond=pred_rust(pred,g);new=f'''        let struct_ty = self.infer_value(flag, depth, env, ctx, structure);\n{raw}\n        let appgen_v25_generated: u8 = {g};\n        let struct_ty_f = if {cond} {{ struct_ty }} else {{ self.force_all(depth, struct_ty) }};'''
elif mode=='g3':new=site
else:raise SystemExit('unknown mode')
s=s.replace(site,new,1);infer.write_text(s)
print(f'V25_RUNTIME_MODE={mode}');print(f'V25_RUNTIME_EXPR={expr or "NONE"}');print(f'V25_RUNTIME_PREDICATE={pred or "NONE"}');print('V25_INITIAL_MEASUREMENT_LANGUAGE=opaque_root+generic_facts+context');print('V25_INTERVENTION_GENERATOR=act+delta+anonymous_indices');print('V25_PASSIVE_TOUCH_SENSE_MICROMACHINE_EXPOSED=NO');print('V25_NAMED_TRAVERSAL_PRIMITIVES_EXPOSED=NO');print('V25_COMBINED_DECOMPOSE_OPERATOR_EXPOSED=NO');print('V25_CELL_SEQUENCE_EXPOSED=NO');print('V25_SLOT_FAMILY_EXPOSED=NO');print('V25_SEMANTIC_NAMES_EXPOSED=NO')
