...
ROOT=/tmp/mda-auto-semantic-v2
CANDIDATE="$(git rev-parse HEAD)"
rm -rf "$ROOT"
mkdir -p "$ROOT/evidence"

echo "MDA_V2_SEMANTIC_CANDIDATE=$CANDIDATE"
...
echo "MDA_V2_SEMANTIC_BUILD_CHECKER=PASS"
nix develop -c ./lka.py build-test --skip-declined-by mathgraph
echo "MDA_V2_SEMANTIC_BUILD_TESTS=PASS"
rm -rf _results
...
payload={"schema":"mda-autonomous-checker-semantic-v2",
...
print("MDA_V2_SEMANTIC_RESULT="+json.dumps(payload,sort_keys=True))
if bad: raise SystemExit(1)
PY
echo "MDA_V2_SEMANTIC_VERDICT=FULL_PINNED_ARENA_PASS"
