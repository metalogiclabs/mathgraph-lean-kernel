...
parts=set(cand.split('_')) if cand != 'g3' else set()
if parts - {'param','prior','rigid'}:
    raise SystemExit(f'unknown candidate {cand}')
if 'param' in parts: s=add_param(s)
if 'prior' in parts: s=add_prior(s)
if 'rigid' in parts: s=add_rigid(s)

infer.write_text(s)
print(f'V10_CANDIDATE={cand}')
print('FROZEN_PREFIX=G1+G2+G3_FINAL_FIELD_PI')
print('DISTANT_SLICE_OUTCOME_UNSEEN_AT_PREDICTION_TIME=YES')
