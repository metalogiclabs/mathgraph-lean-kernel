...
# V2-safe: cache only successful RIGID=true comparisons for ordinary
# same-head rigid values. Recursor/Quot iota paths are intentionally untouched.
...
for old,new in repls:
    if c.count(old)!=1:
        raise SystemExit(f"conv replacement anchor mismatch {old[:70]!r}: {c.count(old)}")
    c=c.replace(old,new,1)

# Guardrail: recursor and quotient paths must remain untouched.
if "return self.unify_short_rigid_spine_cached::<true>(depth, t, t2" in c:
    raise SystemExit("unexpected recursor/iota modification")
if c.count("self.unify_spine::<false>(depth, sx, sy, sig, limit)") != 2:
    raise SystemExit("non-rigid/iota paths unexpectedly changed")

p.write_text(c)
print("QCKN_SHORT_RIGID_CONV_V2_SAFE_PATCH=PASS")
